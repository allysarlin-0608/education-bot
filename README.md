...
- **固定課綱**（`coach/curriculum/<主題>.txt`，一行一課，`#` 開頭是單元名稱）：每個主題規劃 3000 課，
  照順序上，排到的那天上 5 堂，每堂課最後有 10 題小考（7 題選擇、1 題配對、2 題簡答，簡答由 AI 批改；出題後會再請 AI 逐題檢查正解，有問題的題目重出），拿到 8 分（80%）以上這堂才算完成、才能開始下一堂（沒過可以重考，每次換一組新題目），5 堂都通過這一天才算完成；
...
### 使用者設定（科目、每天的量、起始程度、看書計畫）

在 SQL Editor 執行一次 `supabase/user_settings.sql`，建立 `user_settings` 資料表（每位使用者一列，一律用 `user_id` 讀寫；
預設的 user_id 是 `owner`，可以在 Secrets 用 `COACH_USER_ID` 改）。執行之後，下次打開 app 會先出現設定流程
（選科目、每天的量、起始程度、要不要看書計畫），之後可以在 Settings 頁修改。
還沒執行之前，app 照舊用原本固定的設定（每週幾一個科目、一天 5 堂、看書頁一直都在）。
改科目、改每天的量都不會刪掉或改寫任何學習紀錄；程度測驗也不會留下學習紀錄。
